...
This fork adds several custom layers and key behaviours on top of the stock Kinesis config. See **[CUSTOMIZATIONS.md](CUSTOMIZATIONS.md)** for the full usage guide, including complete keyboard layouts and a layer-transition diagram.

| Customization | Summary |
|---|---|
| **Flykey layer** | Hold Space → home-row navigation & editing (no arrow keys needed) |
| **Num layer** | Hold left Backspace → right-hand numpad |
| **Plain layer** | Tap Space in Flykey → disables accidental layer triggers for fast typing |
